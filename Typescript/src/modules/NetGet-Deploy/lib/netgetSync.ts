...
interface CompareResult {
    local: DomainConfig[];
    remote: DomainConfig[];
    onlyLocal: DomainConfig[];
    onlyRemote: DomainConfig[];
    common: DomainConfig[];
}
...
export class NetGetSync {
     ...
     */
    async readLocalConfig(): Promise<DomainConfig[]> {
     ...
     */
    async packageProject(target: string, domain: string): Promise<string> {
     ...
     */
    async syncDomainConfig(domains: DomainConfig[]): Promise<any> {
        try {
            ...
            throw new Error(`Failed to sync domain config: ${axiosError.response?.data || axiosError.message}`);
        }
    }
     ...
     */
    async deployProject(domain: string, zipPath: string): Promise<any> {
        try {
            ...
            throw new Error(`Failed to deploy project: ${axiosError.message}${details}`);
        }
    }
     ...
     */
    async checkRemoteHealth(): Promise<HealthCheckResult> {
        try {
    ...
    async compare(): Promise<CompareResult> {
        try {
            ...
            const remoteDomains: DomainConfig[] = response.data.domains;
