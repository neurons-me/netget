...
--   7. proof.publicKey must match claims.pubkeys[identityHash]
--   8. Ed25519 signature must be valid
--
...
local function sha256_hex(data)
  ...
  digest:update(data or "")
  local bin = digest:final()
...
local function verify_request()
  ...
  -- 4. bodyHash must match sha256(actual request body) — binds the
  -- signature to the payload, not just the endpoint. read_body() must run
  -- before get_body_data() returns anything; nginx doesn't buffer the body
  -- into memory in access_by_lua_file unless told to.
  ngx.req.read_body()
  local actual_body = ngx.req.get_body_data() or ""
  local actual_bodyhash = sha256_hex(actual_body)
  if req_bodyhash ~= actual_bodyhash then
    deny(401, "ME_PROOF_BODY_MISMATCH",
      "Signed bodyHash does not match the actual request body.")
    return
  end
  ...
  -- 8. Identity must be anchored on this gateway.
