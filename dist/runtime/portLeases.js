...
function leasePathForName(name) {
...
async function tryCreatePortLock(port, name) {
...
async function removePortLock(port) {
    await fs.unlink(lockPathForPort(port)).catch(() => { });
}
async function removeLeaseFile(name) {
    await fs.unlink(leasePathForName(name)).catch(() => { });
}
async function releaseStaleLease(lease) {
    await removeLeaseFile(lease.name);
    await removePortLock(lease.port);
}
async function getReusablePortFromExistingLease(name) {
    const existing = await readJson(leasePathForName(name));
    if (!existing)
        return undefined;
...
function assertPortNumber(port) {
...
function createLease(name, port, mode, heartbeatMs) {
    return {
...
async function tryClaimPort(name, port, mode, heartbeatMs) {
...
function buildAutoCandidates(preferredPort) {
    const ports = [];
...
export async function allocatePortLease(name, options = {}) {
    await ensureLeaseDirs();
    const mode = options.mode || (options.preferredPort ? 'fixed' : 'auto');
    ...
    if (mode === 'fixed') {
        ...
        if (!port)
            throw new Error(`Fixed port mode requires a port for '${name}'.`);
        const lease = await tryClaimPort(name, port, mode, heartbeatMs);
        if (!lease)
            throw new Error(`Port ${port} is not available for '${name}'.`);
        return lease;
    }
...
export async function heartbeatPortLease(lease, status = 'active') {
    const next = {
        ...lease,
        pid: process.pid,
        cwd: process.cwd(),
...
export async function releasePortLease(lease) {
    await removeLeaseFile(lease.name);
    await removePortLock(lease.port);
}
export async function listPortLeases() {
    await ensureLeaseDirs();
    ...
    for (const file of files) {
        if (!file.endsWith('.json'))
            continue;
        const lease = await readJson(path.join(portsDir(), file));
        if (!lease)
            continue;
        leases.push({
            ...lease,
