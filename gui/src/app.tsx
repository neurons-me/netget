                ...
                {gateway.status === 'ok' && gateway.authenticated && (
                  ...
                  >
                    ...
                    <Typography variant="caption" sx={{ fontFamily: 'monospace', opacity: 0.6 }}>
                      {gateway.isOwner ? 'Gateway owner authenticated' : 'Gateway admin authenticated'}
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                      {gatewayClaims.owner && (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="caption" sx={{ fontFamily: 'monospace', fontWeight: 700, opacity: 0.9, flex: 1 }}>
                            {gatewayClaims.owner.username ?? gatewayClaims.owner.short + '…'}
                          </Typography>
                          <Box sx={{ fontSize: '9px', fontFamily: 'monospace', px: 0.6, py: 0.15, border: '1px solid', borderColor: 'success.main', borderRadius: 0.75, color: 'success.main', opacity: 0.8, lineHeight: 1.6 }}>
                            owner
                          </Box>
                        </Box>
                      )}
                      {Array.isArray(gatewayClaims.admins) && gatewayClaims.admins.map(admin => (
                        <Box key={admin.hash} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Box sx={{ width: 7, height: 7, borderRadius: '50%', background: 'rgba(255,255,255,0.25)', flexShrink: 0 }} />
                          <Typography variant="caption" sx={{ fontFamily: 'monospace', opacity: 0.75, flex: 1 }}>
                            {admin.username ?? admin.short + '…'}
                          </Typography>
                          <Box sx={{ fontSize: '9px', fontFamily: 'monospace', px: 0.6, py: 0.15, border: '1px solid', borderColor: 'divider', borderRadius: 0.75, opacity: 0.45, lineHeight: 1.6 }}>
                            admin
                          </Box>
                        </Box>
                      ))}
