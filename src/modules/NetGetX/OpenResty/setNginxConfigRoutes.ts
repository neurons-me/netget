...
# ─── NRP Monad Root ───────────────────────────────────────────────────────────
# ${machineHostnameLower} = the monad itself (machine substrate).
# surface_proxy.lua reads apps.json, matches host → registered monad endpoint.
server {
${listenLines}
...
# ─── NRP Handle Surface ───────────────────────────────────────────────────────
# {handle}.${machineHostnameLower} → identity surface for that handle.
# Per NRP: me://handle.space[selector]/path
#   handle  = subdomain captured as $nrp_handle
#   space   = ${machineHostnameLower}
# nrp_handle.lua validates the handle, proxies to local.netget admin panel.
server {
${listenLines}
...
${sslDirectives}
    ...
    location / {
        rewrite_by_lua_file lua/handlers/nrp_handle.lua;
        proxy_pass http://127.0.0.1:80;
        proxy_set_header Host local.netget;
${proxyHeaders}
        proxy_set_header X-NRP-Handle    $nrp_handle;
        proxy_set_header X-NRP-Space     ${machineHostnameLower};
        proxy_set_header X-NRP-Namespace $nrp_handle.${machineHostnameLower};
    }
}
`;
